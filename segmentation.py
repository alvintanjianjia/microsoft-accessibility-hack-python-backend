...
def remove_silence(filename, out_dir, smoothing=1.0, weight=0.3, plot=False):
    ...
    partic_id = 'P' + filename.split('/')[-1].split('_')[0].split('\\')[1]  # PXXX
    ...
    if is_segmentable(partic_id):
        ...
        for s in segments:
            ...
            seg_name = '/' + '_' + str(s[0]).replace('.', 'b') + '_' + str(s[1]).replace('.',
                                                                                                          'b') + '.wav'
            # print(seg_name, 'seg_name')
            wavfile.write(participant_dir + seg_name, Fs, x[int(Fs * s[0]):int(Fs * s[1])])
...
def concatenate_segments(participant_dir, partic_id, remove_segment=True):
    ...
    outfile = participant_dir + '/' + partic_id + '_no_silence.wav'
...
if __name__ == '__main__':
    ...
    dir_name = 'D:/daizwoz_depression_dataset/data/raw/audio'
